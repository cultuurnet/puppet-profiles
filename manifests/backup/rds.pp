...
$config.each |$db_key, $db| {
  ...
  cron { "rds-backup-${db_key}":
    ...
    hour    => '2',
    weekday => '0',
    command => "/usr/bin/mysqldump --host=${host} --user=${user} --password='${password}' --databases $(/usr/bin/mysql --host=${host} --user=${user} --password='${password}' -N -e \"SHOW DATABASES;\" | grep -Ev '^(information_schema|performance_schema|mysql|sys)$') | gzip > ${backupdir}/${db_key}-$(date +\\%F).sql.gz",
  }
}
