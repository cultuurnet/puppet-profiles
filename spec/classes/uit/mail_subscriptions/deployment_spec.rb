describe 'profiles::uit::mail_subscriptions::deployment' do
  ...
  on_supported_os.each do |os, facts|
    ...
    context "with config_source => appconfig/uit/mail-subscriptions/env" do
      let(:params) { {
        'config_source' => 'appconfig/uit/mail-subscriptions/env'
        } }
      ...
      context "with config_source => appconfig/uit/mail-subscriptions/env, version => 1.2.3, repository => foo, service_status => stopped and puppetdb_url => http://example.com:8000" do
        let(:params) { {
          'config_source'  => 'appconfig/uit/mail-subscriptions/env',
          'version'        => '1.2.3',
          'repository'     => 'foo',
        ...
        context "with repository foo defined" do
          ...
          it { is_expected.to contain_file('uit-mail-subscriptions-config').with_content(/key=value/) }
