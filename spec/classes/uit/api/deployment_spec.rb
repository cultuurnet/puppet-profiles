describe 'profiles::uit::api::deployment' do
  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
        ...
        context "with hieradata" do
          ...
          context 'in the testing environment' do
            ...
            it { is_expected.to contain_class('profiles::uit::api::deployment').with(
              ...
              'newrelic_app_name'    => 'uit-api-testing',
            ...
            it { is_expected.to contain_file('uit-api-config-graphql').with_content(/key=value/) }
            it { is_expected.to contain_file('uit-api-config-db').with_content(/key=value/) }
            it { is_expected.to contain_file('uit-api-service-defaults').with_content(/^HOST=127.0.0.1$/) }
            ...
            it { is_expected.to contain_file('uit-api-service-defaults').with_content(/^NEW_RELIC_LICENSE_KEY=my_license_key$/) }
            it { is_expected.to contain_file('uit-api-service-defaults').with_content(/^NEW_RELIC_APP_NAME=uit-api-testing$/) }
            it { is_expected.to contain_file('uit-api-service-defaults').with_content(/^NEW_RELIC_TRACER_ENABLED=false$/) }
            it { is_expected.to contain_file('uit-api-service-defaults').with_content(/^NEW_RELIC_DISTRIBUTED_TRACING_ENABLED=false$/) }
      ...
      context "with config_source => appconfig/uit/api/env, maximum_heap_size => 1024, service_port => 3456, version => 1.2.3, repository => uit-api-exotic, service_status => stopped, newrelic_license_key => ping, newrelic_app_name => pong, newrelic_tracing => true and puppetdb_url => http://example.com:8000" do
        let(:params) { {
          'config_source'        => 'appconfig/uit/api/env',
          'version'              => '1.2.3',
        ...
        context "with hieradata" do
          ...
          context "with repository uit-api-exotic defined" do
            ...
            it { is_expected.to contain_file('uit-api-service-defaults').with_content(/^NEW_RELIC_TRACER_ENABLED=true$/) }
            it { is_expected.to contain_file('uit-api-service-defaults').with_content(/^NEW_RELIC_DISTRIBUTED_TRACING_ENABLED=true$/) }
