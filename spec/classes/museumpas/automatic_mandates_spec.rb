describe 'profiles::museumpas::automatic_mandates' do
  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'api_url => https://museumpas.example.com and jwt_token => abc' do
        ...
        it { is_expected.to contain_class('profiles::museumpas::automatic_mandates').with(
          'ensure'    => 'present',
          'api_url'   => 'https://museumpas.example.com',
          'jwt_token' => 'abc',
          'hour'      => '0',
        ...
        it { is_expected.to contain_cron('profiles::museumpas::automatic_mandates').with(
          'ensure'      => 'present',
          'environment' => [ 'MAILTO=infra@publiq.be'],
          'command'     => "/usr/bin/curl -X 'POST' -H 'Authorization: Bearer abc' https://museumpas.example.com/rest/system/autorenewalInspectPending",
          'hour'        => 0,
          'minute'      => 0
        ) }
      end
      ...
      context 'api_url => https://foo.example.com, jwt_token => def, hour => 3 and minute => 14' do
        ...
        it { is_expected.to contain_cron('profiles::museumpas::automatic_mandates').with(
          'ensure'      => 'present',
          'environment' => [ 'MAILTO=infra@publiq.be'],
          'command'     => "/usr/bin/curl -X 'POST' -H 'Authorization: Bearer def' https://foo.example.com/rest/system/autorenewalInspectPending",
          'hour'        => 3,
          'minute'      => 14
        ) }
      end
