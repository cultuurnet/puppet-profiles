      ...
      context "without parameters" do
        ...
        it { is_expected.to contain_class('apache').with(
          ...
          'log_formats'           => {
                                       ...
                                       'api_key_json'  => '{ \"remoteIP\": \"%{CLIENT_IP}e\", \"time\": \"%{%Y-%m-%d %H:%M:%S}t.%{msec_frac}t\", \"requestPath\": \"%U\", \"status\": \"%>s\", \"query\": \"%q\", \"method\": \"%m\", \"userAgent\": \"%{User-Agent}i\", \"referer\": \"%{Referer}i\", \"uniqueID\": \"%{UNIQUE_ID}e\", \"duration\": \"%{ms}T\", \"apiKey\": \"%{API_KEY}e\", \"jwtToken\": \"%{JWT_TOKEN}e\", \"clientProperties\": \"%{X-Client-Properties}i\" }'
                                     }
        ) }
