    ...
    context "on #{os}" do
      ...
      context 'with hieradata' do
        ...
        context "with user => john, password => doe and url => 'https://jenkins.example.com/'" do
          ...
          it { is_expected.to contain_profiles__jenkins__node_labels('parameter').with(
            'content' => []
          ) }

          case facts[:os]['release']['major']
          when '20.04'
            it { is_expected.to contain_profiles__jenkins__node_labels('os').with(
              'content' => ['Ubuntu', '20.04', 'focal']
            ) }
          when '24.04'
            it { is_expected.to contain_profiles__jenkins__node_labels('os').with(
              'content' => ['Ubuntu', '24.04', 'noble']
            ) }
          end

          it { is_expected.to contain_concat('jenkins-swarm-client_node-labels').with(
            'owner'          => 'jenkins',
            'group'          => 'jenkins',
          ...
          it { is_expected.to contain_file('jenkins-yarn-cache-cleanup').with_content(%r{node_names=\("\$\(hostname -s\)" "\$\(hostname -f\)"\)}) }
          ...
          it { is_expected.to contain_concat('jenkins-swarm-client_node-labels').that_requires('User[jenkins]') }
          it { is_expected.to contain_concat('jenkins-swarm-client_node-labels').that_requires('Group[jenkins]') }
          it { is_expected.to contain_concat('jenkins-swarm-client_node-labels').that_requires('Package[jenkins-swarm-client]') }
