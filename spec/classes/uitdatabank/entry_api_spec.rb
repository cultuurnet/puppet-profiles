  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'with hieradata' do
        ...
        context 'with database_password => mypassword, servername => uitdatabank.example.com, job_interface_servername => jobs.example.com, type => container and deployment => false' do
          let(:params) { {
            'database_password'        => 'mypassword',
            'servername'               => 'uitdatabank.example.com',
            'job_interface_servername' => 'jobs.example.com',
          ...
          it { is_expected.to contain_profiles__apache__vhost__reverse_proxy('http://uitdatabank.example.com').with_additional_rewrites(
            [{
              'comment'      => 'Capture apiKey from URL parameters',
              'rewrite_cond' => '%{QUERY_STRING} (?:^|&)apiKey=([^&]+)',
              'rewrite_rule' => '^ - [E=API_KEY:%1]'
