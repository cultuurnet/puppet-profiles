describe 'profiles::uitdatabank::frontend::deployment::instance' do
  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'with config_source => appconfig/uitdatabank/udb3-frontend/env' do
        let(:params) { {
        ...
        context "with hieradata" do
          ...
          it { is_expected.to contain_profiles__deployment__versions('profiles::uitdatabank::frontend::deployment::instance').with(
            'puppetdb_url' => 'http://localhost:8081'
          ) }

          it { is_expected.to contain_package('uitdatabank-frontend').that_notifies('Service[uitdatabank-frontend]') }
          it { is_expected.to contain_package('uitdatabank-frontend').that_notifies('Profiles::Deployment::Versions[profiles::uitdatabank::frontend::deployment::instance]') }
      ...
      context 'with config_source => appconfig/uitdatabank/udb3-frontend/myenv, version => 1.2.3, repository => uitdatabank-frontend-alternative, service_status => stopped, service_address => 0.0.0.0 and service_port => 6000 ' do
        let(:params) { {
          'config_source'   => 'appconfig/uitdatabank/udb3-frontend/myenv',
          'version'         => '1.2.3',
