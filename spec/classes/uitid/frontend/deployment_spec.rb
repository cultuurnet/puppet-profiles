describe 'profiles::uitid::frontend::deployment' do
  ...
  context "with config_source => /bar, maximum_heap_size => 1024, service_address => 0.0.0.0, service_port => 3456, version => 1.2.3, repository => uitid-frontend-exotic, service_status => stopped and puppetdb_url => http://example.com:8000" do
    let(:params) { {
      'config_source'     => '/bar',
      'version'           => '1.2.3',
    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context "with repository uitid-frontend-exotic defined" do
          ...
          it { is_expected.to contain_file('uitid-frontend-config').with('source' => '/bar') }
