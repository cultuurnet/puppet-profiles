...
describe Facter::Util::Fact do
  ...
  describe 'balie_version' do
    ...
    context 'with package balie:20200729132510, balie-angular-app:20200729132510+sha.e3c29b6 and balie-silex:20200728152530+sha.a926ff3' do
      before :each do
        expect(Facter::Util::Resolution).to receive(:exec).with("dpkg-query -f='${binary:Package}:${Version}\\n' -W 'balie*' 2> /dev/null") {
      ...
      it do
        expect(Facter.fact(:balie_version).value).to eq(
          {
            "balie" => { "version" => "20200729132510", "pipeline" => "20200729132510" },
