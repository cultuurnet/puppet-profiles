...
describe Facter::Util::Fact do
  ...
  describe 'museumpas-website_version' do
    ...
    context 'with package museumpas-website:2020.06.09.124800+sha.692e9e0 and no custom metadata fields' do
      before :each do
    ...
    context 'with package museumpas-website:2020.06.09.124800+sha.692e9e0 and custom metadata fields' do
      before :each do
        expect(Facter::Util::Resolution).to receive(:exec).with("dpkg-query -f='${binary:Package}:${Version}:${Pipeline-Version}:${Git-Ref}\\n' -W 'museumpas-website*' 2> /dev/null") { 'museumpas-website:2020.06.09.124800+sha.692e9e0:2020.06.09.124800:692e9e0' }
      end
    ...
    context 'with package museumpas-website-database:2018.08.10.152730, museumpas-website-files:2018.08.16.150116 and museumpas-website:2020.06.09.124800+sha.692e9e0 and no custom metadata files' do
      before :each do
        expect(Facter::Util::Resolution).to receive(:exec).with("dpkg-query -f='${binary:Package}:${Version}:${Pipeline-Version}:${Git-Ref}\\n' -W 'museumpas-website*' 2> /dev/null") {
    ...
    context 'with package museumpas-website-database:2018.08.10.152730, museumpas-website-files:2018.08.16.150116 and museumpas-website:2020.06.09.124800+sha.692e9e0 and custom metadata files' do
      before :each do
        expect(Facter::Util::Resolution).to receive(:exec).with("dpkg-query -f='${binary:Package}:${Version}:${Pipeline-Version}:${Git-Ref}\\n' -W 'museumpas-website*' 2> /dev/null") {
          "museumpas-website-database:2018.08.10.152730::\nmuseumpas-website-files:2018.08.16.150116:2018.08.16.150116:\nmuseumpas-website:2020.06.09.124800+sha.692e9e0:2020.06.09.124800:692e9e0"
        }
      end
